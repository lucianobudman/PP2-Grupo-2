  ...
  try {
    ...
    for (const item of items) {
      const producto = item.productoId
        ? await Producto.findByPk(item.productoId)
        : await Producto.findOne({ where: { nombre: item.nombre } });

      if (!producto) {
        return res.status(404).json({ error: `Producto no encontrado: ${item.nombre || item.productoId}` });
      }
    ...
    for (const { producto, cantidad } of productosValidados) {
      ...
      detalles.push({ productoId: producto.id, cantidad, precio_unitario: producto.precio });
      await Producto.update({ stock: producto.stock - cantidad }, { where: { id: producto.id } });
    }
