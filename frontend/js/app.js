...
function esClienteAdmin() {
  return clienteActivo && (clienteActivo.isAdmin === true || clienteActivo.isAdmin === 1 || clienteActivo.isAdmin === '1' || clienteActivo.isAdmin === 'true');
}

function actualizarVistaAdmin() {
  const botonAdmin = document.getElementById('btn-admin');
  const panelAdminHome = document.getElementById('panel-admin-home');

  if (!botonAdmin && !panelAdminHome) return;

  const adminVisible = esClienteAdmin();

  if (botonAdmin) {
    botonAdmin.style.display = adminVisible ? 'inline-flex' : 'none';
  }
      ...
      <div class="card h-100 p-3">
        ...
        <div class="card-body p-0">
          ...
          <button class="btn-agregar"
            ...
            onclick="agregarAlCarrito('${nombreEscapado}', ${producto.precio}, 'cant-${index}')">
...
async function cargarPanelAdmin() {
  try {
    const [productosRes, clientesRes, ordenesRes] = await Promise.all([
      fetch('/api/productos'),
      fetch('/api/clientes'),
      fetch('/api/ordenes')
    ]);
    ...
    document.getElementById('admin-productos').textContent = productos.length;
    document.getElementById('admin-clientes').textContent = clientes.length;
    document.getElementById('admin-ordenes').textContent = ordenes.length;

    const body = document.getElementById('admin-ordenes-body');
    body.innerHTML = '';

    if (!ordenes.length) {
      body.innerHTML = '<tr><td colspan="4" style="opacity:0.6;">No hay órdenes aún.</td></tr>';
      return;
    }
    ...
    ordenes.slice(0, 5).forEach(orden => {
      ...
      tr.innerHTML = `
        <td>#${orden.id}</td>
        <td>${orden.clienteId || 'Sin cliente'}</td>
        <td>$${Number(orden.total || 0).toLocaleString('es-AR')}</td>
